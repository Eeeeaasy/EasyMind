<template>
  <section class="page page-debug">
    ...
    <div class="debug-layout">
      <section class="workspace-card"><div class="card-heading"><div><span class="kicker">Backend</span><h2>后端连接</h2></div><span class="status-copy" :class="healthOk ? 'success' : 'muted'">{{ healthLabel }}</span></div><div class="backend-tabs"><button :class="{ active: settings.backend === 'java' }" @click="$emit('switch-backend', 'java')">Java</button><button :class="{ active: settings.backend === 'python' }" @click="$emit('switch-backend', 'python')">Python</button></div><label><span>Java API</span><input v-model="settings.endpoints.java" @change="$emit('persist')" placeholder="/api/java" /></label><label><span>Python API</span><input v-model="settings.endpoints.python" @change="$emit('persist')" placeholder="/api/python" /></label><div class="side-actions"><button @click="$emit('check-health')">检查连接</button><button class="quiet-button" @click="$emit('refresh')">刷新状态</button></div></section>
...
defineEmits(['switch-backend', 'persist', 'check-health', 'refresh'])
</script>
