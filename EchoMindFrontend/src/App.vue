...
const theme = ref(
  localStorage.getItem("echomind-theme") === "light" ? "light" : "dark",
);
const messages = ref([]);
const draft = ref("");
...
const userInitial = computed(() =>
  (settings.userId || "U").slice(0, 1).toUpperCase(),
);
const activeAlerts = computed(() => monitorData.value.active_alerts || []);
const agentCount = computed(
  () => Object.keys(monitorData.value.agent_stats || {}).length,
);
