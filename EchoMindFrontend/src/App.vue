  ...
  <main :class="['app-shell', `app-shell-${activeView}`, `theme-${theme}`]">
    <header class="topbar">
      ...
      <nav class="view-nav" aria-label="工作区">
        ...
        <button :class="{ active: activeView === 'legacy' }" @click="activeView = 'legacy'">扩展模块</button>
      </nav>
    ...
    <section v-else-if="activeView === 'evaluation'" class="page page-evaluation">
      <div class="page-heading">
        <div class="heading-copy">
    ...
    <section v-else class="page page-legacy">
      <div class="page-heading">
        <div class="heading-copy">
          <span class="kicker">Legacy modules</span>
          <h1>扩展调试模块</h1>
          <p>保留旧版的调试、RAG 检索、知识导入、监控和评测能力。</p>
        </div>
      </div>
      <LegacyModules />
    </section>
...
import LegacyModules from './LegacyModules.vue'
import {
  addKnowledge,
  backendMeta,
...
const theme = ref(localStorage.getItem('echomind-theme') === 'light' ? 'light' : 'dark')
const messages = ref([])
const draft = ref('')
...
function toggleTheme() {
  ...
  localStorage.setItem('echomind-theme', theme.value)
}
